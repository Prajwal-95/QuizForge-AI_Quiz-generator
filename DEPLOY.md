...
**You're live** (use the URLs shown on your actual service pages — Render appends a
random 4-letter suffix. The current deployment is:):

- Frontend → `https://quizforge-jybu.onrender.com`
- Backend → `https://quizforge-api-ntm4.onrender.com`
- API docs → `https://quizforge-api-ntm4.onrender.com/docs`

### ⚠️ Post-deploy: make it LIVE (required for real AI generation)

The app is **designed** to stay in Demo Mode until you add a Groq API key:
whenever `GROQ_API_KEY` is missing, `/generate` serves the curated Operating
Systems demo question bank instead of real LLM questions, and the UI badge shows
**Demo mode**. This is not a bug — it's the fallback kicking in.

To go live:

1. Create a free API key at <https://console.groq.com> → **API Keys** → **Create**.
...
4. In the app, the sidebar badge flips from **Demo mode** → **Connected** and the
   ...
   real Groq-backed questions.

> `JWT_SECRET` is auto-generated by the blueprint — no action needed there.

### 🗄️ Accounts persistence (login "Invalid email or password")

Render free-tier containers are **ephemeral**: any file written inside the
container (including the SQLite database at `./quizforge.db`) is destroyed when
the instance stops after ~15 minutes idle or restarts. That's why accounts kept
disappearing. The `render.yaml` blueprint now mounts a **persistent disk** and
stores the DB at `/var/data/quizforge.db` (`DATABASE_URL`).

**After pulling this change, re-apply the blueprint** so the disk gets created:

1. Render dashboard → **Blueprints** → your blueprint →
   **Update / Sync** (shows the disk + env-var diff) → **Apply**.
2. Both services redeploy. Accounts and quizzes now survive restarts.

> Already-created accounts from before the disk won't come back (they were
> deleted with the old containers). Just create the account once more —
> this time it stays.

### 🔑 Optional: Sign in with Google

The login modal shows a **Continue with Google** button only when a Google
OAuth client ID is configured on BOTH services:
...
1. Go to <https://console.cloud.google.com/apis/credentials> → **Create
   ...
   - **Authorized JavaScript origins**: add your frontend URL
     (e.g. `https://quizforge-jybu.onrender.com`).
2. Backend service (`quizforge-api`) → **Environment** →
   `GOOGLE_CLIENT_ID` = `<that client ID>` → Save.
3. Frontend service (`quizforge`) → **Environment** →
   `VITE_GOOGLE_CLIENT_ID` = the **same** client ID → Save (rebuilds).

Leave either one blank and the button is simply hidden (email/password still
works). The backend verifies Google's ID token against Google's public certs and
...
| Key | Value |
|-----|-------|
| `GROQ_API_KEY` | your Groq API key — **required** for live AI generation; without it the app stays in Demo Mode (local question bank) |
...
6. **Environment variable** (available during build):
   - `VITE_API_BASE` = `https://<your-backend-name>.onrender.com/api`
     (use your actual backend URL, e.g. `https://quizforge-api-ntm4.onrender.com/api`)
...
- Backend API docs: `https://quizforge-api-ntm4.onrender.com/docs`
- ⚠️ Finish the **"make it LIVE"** step above (add `GROQ_API_KEY`) or the app
  keeps generating from the demo question bank.
